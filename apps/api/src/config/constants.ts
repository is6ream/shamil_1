...
/**
 * Способы, которыми поступает ручной перевод. Значение ложится в `donation.method`
 * и показывается в живой ленте поступлений наравне с СБП и картой: жертвователь,
 * принёсший деньги наличными, в ленте выглядит так же, как оплативший онлайн.
 */
export const MANUAL_CONFIRM_METHODS = ['bank_transfer', 'sbp', 'cash'] as const;

export type ManualConfirmMethod = (typeof MANUAL_CONFIRM_METHODS)[number];

/** Перевод по реквизитам — основной сценарий запасного пути. */
export const MANUAL_CONFIRM_DEFAULT_METHOD: ManualConfirmMethod = 'bank_transfer';

/**
 * Потолок суммы, которую админ подтверждает вручную, — 240 млн ₽ в копейках,
 * то есть вся цель сбора целиком.
 *
 * `MAX_DONATION_KOPECKS` здесь не годится: он ограничивает *онлайн*-донат против
 * опечатки в публичной форме, а по реквизитам приходят и суммы крупнее. Но
 * граница нужна всё равно — лишний ноль в админской форме попадёт прямо
 * в витринные счётчики, и убрать его оттуда можно будет только правкой
 * оплаченного доната.
 */
export const MAX_MANUAL_CONFIRM_KOPECKS = 24_000_000_000n;

/**
 * Минимальная длина админского токена. 32 символа — это не «достаточно длинно
 * на глаз»: токен статический, живёт месяцами и защищает эндпоинт, который
 * двигает сумму сбора.
 */
