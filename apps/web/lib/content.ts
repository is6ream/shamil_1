...
/** Заголовки и лиды страниц `/otchety` и `/galereya`. */
export const REPORTS_PAGE = {
  title: "Отчёты и поступления",
  lede:
    "Все пожертвования на строительство — время уфимское. " +
    "Отчёты о расходах по этапам появятся здесь же.",
} as const;

export const GALLERY_PAGE = {
  lede: "От заливки фундамента до купола — снимки с объекта по порядку.",
} as const;
